...
impl TreeFormatter {
    ...
    fn format_module(&self, module: &NodeModuleInfo, output: &mut String, prefix: &str, is_last: bool) {
        ...
        let version_str = if let Some(version) = &module.version {
            if self.config.no_color {
                ...
                format!("@{}", version.dimmed().to_string())
            }
        } else {
            String::new()
        };
        ...
        if !module.constants.is_empty() {
            let const_names: Vec<String> = module.constants.iter().map(|c| c.name.clone()).collect();
