...
impl NpmClient {
    ...
    pub async fn get_package_info(
        ...
        let pkg_data = if let Some(versions) = data.get("versions") {
            ...
            if let Some(v) = version {
                versions
                    .get(v)
                    .ok_or_else(|| anyhow!("Version {} not found", v))?
            } else {
    ...
    pub fn find_local_package<P: AsRef<Path>>(
        &self,
        package_name: &str,
        search_paths: &[P],
    ) -> Option<PathBuf> {
