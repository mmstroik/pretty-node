...
impl TypeScriptParser {
    ...
    pub fn parse_declaration_file(&self, file_path: &Path) -> Result<NodeModuleInfo> {
        ...
        self.parse_declaration_content(&content, file_path.file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("unknown"))
    }
    
    /// Parse TypeScript declaration content
    pub fn parse_declaration_content(&self, content: &str, module_name: &str) -> Result<NodeModuleInfo> {
    ...
    fn extract_function_declarations(&self, content: &str, module_info: &mut NodeModuleInfo) {
        let func_regex = regex::Regex::new(r"(?m)^export\s+(?:declare\s+)?function\s+(\w+)\s*\(([^)]*)\)").unwrap();
