...
impl NodeModuleExplorer {
    ...
    pub async fn explore(&self) -> Result<NodeModuleInfo> {
        ...
        if let Some(local_path) = self.npm_client.find_local_package(&package_name, &search_paths) {
            if !self.quiet {
        ...
        let package_info = self.npm_client.get_package_info(&package_name, version.as_deref()).await?;
        let temp_dir = self.npm_client.download_package(&package_info, self.quiet).await?;
        
        let package_path = temp_dir.path().join("package");
        self.explore_local_package(&package_path, &package_name).await
    }
    
    async fn explore_local_package(&self, package_path: &Path, package_name: &str) -> Result<NodeModuleInfo> {
        ...
        if self.max_depth > 1 {
            self.explore_submodules(package_path, &mut root_module, 1).await?;
        }
        
        Ok(root_module)
    }
    
    fn parse_package_json(&self, package_json_path: &Path, module_info: &mut NodeModuleInfo) -> Result<()> {
    ...
    fn find_entry_points(&self, package_path: &Path, module_info: &NodeModuleInfo) -> Result<HashMap<String, PathBuf>> {
    ...
    async fn explore_submodules(&self, package_path: &Path, parent_module: &mut NodeModuleInfo, current_depth: usize) -> Result<()> {
        ...
        for subdir in subdirs_to_check {
            ...
            if subdir_path.exists() && subdir_path.is_dir() {
                self.explore_directory(&subdir_path, parent_module, current_depth).await?;
            }
        }
    ...
    async fn explore_directory(&self, dir_path: &Path, parent_module: &mut NodeModuleInfo, current_depth: usize) -> Result<()> {
        if current_depth >= self.max_depth {
            return Ok(());
        }
